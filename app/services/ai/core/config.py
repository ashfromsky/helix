...
class AISettings(BaseSettings):
    ...
    AI_PROVIDER: AIProvider = Field(
        default=AIProvider.DEMO, description="AI provider to use (deepseek, ollama, groq, demo)"
    )

    # DeepSeek (OpenRouter)
    OPENROUTER_API_KEY: Optional[str] = Field(default=None, description="OpenRouter API key for DeepSeek")
    OPENROUTER_MODEL: str = Field(default="deepseek/deepseek-chat", description="Model to use on OpenRouter")
    ...
    AI_MAX_TOKENS: int = Field(default=2000, gt=0, description="Maximum tokens in response")
    AI_TIMEOUT: int = Field(default=30, gt=0, description="Request timeout in seconds")

    AI_AUTO_FALLBACK: bool = Field(default=True, description="Auto fallback to demo mode if AI fails")
